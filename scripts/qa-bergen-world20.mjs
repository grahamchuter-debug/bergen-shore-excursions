/**
 * Bergen World 2.0 destination-master QA.
...
const dataPath = path.join(root, "src/data/bergen-cruise-schedules.generated.json");
if (!existsSync(dataPath)) {
  fail("missing bergen-cruise-schedules.generated.json");
  process.exit(1);
}
...
if (integrity.total !== expected.total || rows.length !== expected.total) {
  ...
  pass(`total Bergen calls ${expected.total}`);
}
...
const required = [
  ...
  "src/app/bergen-city-or-fjord/page.tsx",
...
if (!config.includes("bergenshoreexcursions.com")) {
  fail("canonical domain missing from site-config");
} else {
  pass("canonical domain bergenshoreexcursions.com present");
}
...
if (!config.includes("hello@bergenshoreexcursions.com")) {
  fail("public contact email missing");
} else {
  pass("public contact email hello@bergenshoreexcursions.com present");
}

console.log(
  "\nINFO: sync source = norway-shore-excursions generated JSON, filter port===bergen",
);
if (process.exitCode) {
  console.error("\nQA FAILED");
  process.exit(1);
