/**
 * Sync Bergen schedule subset from Norway authority master JSON.
 *
 * SOURCE: ../norway-shore-excursions/src/data/cruise-schedules.generated.json
 * FILTER: rows where port === "bergen"
 * OUTPUT: src/data/bergen-cruise-schedules.generated.json
 * METHOD: build-time / manual node script (static export compatible)
 */
...
const outPath = path.join(root, "src/data/bergen-cruise-schedules.generated.json");

const payload = JSON.parse(readFileSync(sourcePath, "utf8"));
const rows = (payload.rows || []).filter((r) => r.port === "bergen");

if (rows.some((r) => String(r.arrival_date || "").startsWith("2028"))) {
  console.error("ABORT: 2028 rows found in Bergen filter");
  process.exit(1);
}
...
console.log("Synced Bergen schedules");
