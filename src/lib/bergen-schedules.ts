import schedulePayload from "@/data/bergen-cruise-schedules.generated.json";

export type BergenScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
export type BergenScheduleEntry = {
  date: string;
  ship: string;
  arrival: string;
  departure: string;
...
type SchedulePayload = {
  ...
  rows: BergenScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const bergenScheduleIntegrity = payload.integrity;
export const bergenScheduleMeta = {
  port: payload.port,
...
function toEntry(row: BergenScheduleRow): BergenScheduleEntry {
  return {
...
export function getAllBergenRows(): BergenScheduleRow[] {
  return payload.rows;
}

export function getBergenEntries(): BergenScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getBergenEntriesForMonthKey(monthKey: string): BergenScheduleEntry[] {
  return payload.rows
...
export function getBergenMonthKeysWithCalls(): string[] {
  return [...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7)))].sort();
}

export function getBergenMonthSummaries() {
